...
make.fits.with.all.models.for.one.gene = function(T = T, gene.index = 1, debug = FALSE, parametrization = c('cosine.beta'), zt = seq(0,46,by = 2), i.ex = ZT.ex, i.int = ZT.int, absolute.signal = TRUE)
{
	...
	for(model in 1:4)
	{
		...
		param.fit = make.fit.spec.model(T = T, gene.index = gene.index, model = model, debug = debug, zt = zt, i.ex = i.ex, i.int = i.int);
...
make.fit.spec.model = function(T = T, gene.index = 1, model = 1, debug = FALSE, parametrization = c('cosine.beta'),zt = seq(0,46,by = 2), i.ex = ZT.ex, i.int = ZT.int, absolute.signal = TRUE)
{
	param.fit = NA
	S = unlist(T[gene.index, i.int])
	M = unlist(T[gene.index, i.ex])
    
    noise.estimate.gene.specific.with.replicates = TRUE
    if(noise.estimate.gene.specific.with.replicates)
    {
        sigma.ss = variance.estimate.replicates(log(S), nb.replicates=4)
        sigma.mm = variance.estimate.replicates(log(M), nb.replicates=4)
    }
...
{
    load(file=paste('myRdata/my_genes_RNA_seq_analysis_variance_mean_logRPKM_loess_v1.Rdata', sep=''))
...
####################################################
make.optimization = function(T = T, i = 1, model = 2, Nfit = NA, debug = FALSE, parametrization =c('cosine.beta'), zt =  seq(0,46,by = 2), i.ex = ZT.ex, i.int = ZT.int, absolute.signal = TRUE)
{
	...
	if(absolute.signal) 
	{
        ...
        ##### compute the variance (sd here) with prediction of LOESS
		...
		amp.gamma.init = sample(lseq(0.01, 20, length = 100), Nfit, replace = TRUE)
		...
		{	
			...
			PAR.INIT = cbind(gamma.init, amp.gamma.init, phase.gamma.init, splicing.k.init, Min.init)
			colnames(PAR.INIT) = c('gamma','amp.gamma','phase.gamma','splicing.k', 'Min.int');
		...
		if(model==4)
		{
			...
			PAR.INIT = cbind(gamma.init, amp.gamma.init, phase.gamma.init, splicing.k.init, Min.init, Amp.init, phase.init, beta.init); 
			colnames(PAR.INIT) = c('gamma','amp.gamma','phase.gamma','splicing.k', 'Min.int','Amp.int','phase.int','beta.int')
			upper[4] = log(2)/(1/60/60) ## maximum splicing rate of 1s
            lower[5] = min(S)/5;
			upper[5] = max(S);
        ...
        while(fit.number<(Nfit+1))
		{
            ...
            opt = opt1;
            #ptm <- proc.time()
            #opt2 = GenSA(par.init, f2min, M = M, S = S, sigma.ss=sigma.ss, sigma.mm=sigma.mm, model = model, debug = debug, zt = zt, lower = lower, upper = upper, control = list(maxit=500, max.time=10*60))
            #opt2 = nls.lm(par.init, lower = lower, upper = upper, f2min, M, S, sigma.ss, sigma.mm, model, debug, zt, jac = NULL, control = nls.lm.control(maxiter = 500))
            #opt2 = GenSA(pars, f2min, data=percents, lambda=lambda, lower = lower, upper = upper,  )
            #proc.time() - ptm
            #alternative.optim = TRUE
            #if(alternative.optim)
            #{
            #   library(GenSA)
            #   opt2 = GenSA(par.init, f2min, M = M, S = S, sigma.ss=sigma.ss, sigma.mm=sigma.mm, model = model, debug = debug, zt = zt, lower = lower, upper = upper, control = list(maxit=500, max.time=10*60))
            ...
            ### check and change the upper boundary of half-life (gamma)
            if(abs(res.fit[1]-upper[1])<0.00001 & upper[1]<log(2)/(1/60))
            {
                cat('\t\t\t increase upper boundary for gamma', '\n');
                upper[1] = upper[1]*2;
                cat('\t\t\t fit start over', '\n');
                fit.number = 1;
            }
...
set.bounds = function(model = 4, parametrization =c('cosine.beta'), absolute.signal = TRUE)
{
	...
	gamma.min = log(2)/24
	gamma.max = log(2)/(8/60);

	### amplitudes of gamma
	amp.gamma.min = 0;
	amp.gamma.max = gamma.max
	...
	if(absolute.signal)
	{
		...
		if(model==3)
		{
			upper = c(gamma.max, amp.gamma.max, phase.gamma.max, splicing.k.max, Min.int.max)
			lower = c(gamma.min, amp.gamma.min, phase.gamma.min, splicing.k.min, Min.int.min)
		}
		if(model==4)
		{
			upper = c(gamma.max, amp.gamma.max, phase.gamma.max, splicing.k.max,  param.synthesis.upper)
			lower = c(gamma.min, amp.gamma.min, phase.gamma.min, splicing.k.min,  param.synthesis.lower)
		}
	}
...
f2min = function(par, M, S, sigma.ss=rep(0.2, length(S)), sigma.mm=rep(0.1, length(M)), model, parametrization =c('cosine.beta'), debug = FALSE, zt = seq(0,46,by = 2), absolute.signal = TRUE)
{
	...
	if(absolute.signal)
	{
		...
		#print(par);
		#print(compute.m.beta(t = zt, gamma, amp.gamma, phase.gamma, splicing.k, param.synthesis.1, param.synthesis.2, param.synthesis.3, param.synthesis.4, simulation.only=TRUE))
		m = compute.m.beta(t = zt, gamma, amp.gamma, phase.gamma, splicing.k, 
...
############
#### Integration 
integrate.m = function(t=seq(0, 46, by=2), gamma=log(2)/5, amp.gamma=1.0, phase.gamma=12, splicing.k=log(2)/(5/60), Min = 0.5, Amp=2.0, phase=12, beta=1)
{
	w <<- 2*pi/24;
	m = rep(0, length(t))
	...
	{
		time = t[i]; 
		m[i] = splicing.k*exp(-Gamma(t = Tstable+time, gamma = gamma, amp.gamma=amp.gamma, phase.gamma=phase.gamma)) * integrate(f2integrate, lower = 0, upper = Tstable+time, par = c(gamma, amp.gamma, phase.gamma, Min, Amp, phase, beta))$value	
	}
	return(m)
}
f2integrate = function(t, par)
{
	...
	phase = par[6];
	beta = par[7];
	return(exp(Gamma(t, gamma, amp.gamma, phase.gamma)) * compute.s.beta(t, Min, Amp, phase, beta))
}
#### This is the integral of degradation function
Gamma = function(t = 0, gamma = log(2)/3, amp.gamma = 0, phase.gamma = 0)
{
	w <<- 2*pi/24;
...
dmdt = function(t, y, par)
{
	...
	gamma.t = gamma + amp.gamma*(1+cos(2*pi/24*(t-phase.gamma)))/2.0
...
compute.m.beta = function(t=seq(0, 94, by=2), gamma=log(2)/5, amp.gamma=0.5, phase.gamma=12, splicing.k=log(2)/(5/60), Min = 0.5, Amp=5, phase=12, beta=1, simulation.only=FALSE)
{
	#gamma=log(2)/5; eps.gamma=1.0; phase.gamma=12; splicing.k=log(2)/(5/60); mean = 10; fold.change=10; phase=6; beta=1;t = seq(0,46, by=2);simulation.only=FALSE
    #gamma=4.159; amp.gamma=100; phase.gamma=24; splicing.k=43.135; Min = 0.01418; Amp=0.215; phase=6.3; beta=5;t = seq(0,94, by=2);simulation.only=FALSE
	w <<- 2*pi/24;
	zt = t;
	...
	if(!simulation.only)
	{
		m.try = try(integrate.m(t = zt, gamma, amp.gamma, phase.gamma, splicing.k, Min, Amp, phase, beta), silent = TRUE)
		if(!inherits(m.try, "try-error")){
            ...
            #cat(zt, '\n')
            #cat(c(gamma, amp.gamma, phase.gamma, splicing.k, Min, Amp, phase, beta), '\n')
			#print(c(gamma, amp.gamma, phase.gamma, splicing.k, Min, Amp, phase, beta));
			m = simulate.m(t = zt, par = c(gamma, amp.gamma, phase.gamma, splicing.k, Min, Amp, phase, beta));
		}
	}else{		
		#cat('HERE')
		m = simulate.m(t = zt, par = c(gamma, amp.gamma, phase.gamma, splicing.k, Min, Amp, phase, beta));
...
compare.parameters.RNA.seq.total.fake = function(T = T, select.method='BIC.best.model', parametrization = c('consine.beta'), quality = FALSE)
{
	...
	parameter.list = c('Min.int','Amp.int','phase.int','beta.int', 'gamma','amp.gamma','phase.gamma','splicing.k')
